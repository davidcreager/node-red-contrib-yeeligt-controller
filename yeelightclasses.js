...
function Yeelight(address, port){
  ...
  .connect(port, address, function(err){
		...
		this.sync()
			...
			.catch( function(err){
				console.log("[Yeelight2][connect] Not Emitting connect emitting error");
				this.emit('error', err);
			}.bind(this) );
	}.bind(this));
  return this;
...
Yeelight.prototype.close = function(){
  this.socket.end();
  this.socket.destroy();
  this.emit('disconnect', this, "closed");
}
  ...
  request.promise = new Promise((accept, reject) => {
    ...
    this.socket.write(message + '\r\n', err => {
      ...
      var timeout = setTimeout(function(){
		...
		if(!respond) reject('Network timeout, Yeelight not response');
      }, 5000);
...
Yeelight.prototype.exit = function(){
  this.socket.end();
  this.emit('disconnect', this, "exited");
  return this;
};
